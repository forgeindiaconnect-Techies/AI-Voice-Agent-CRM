  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] overflow-hidden font-sans">
        ...
        <div className="fixed inset-y-0 right-0 flex z-[10000]">
          <motion.div
            initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            transition={{ type: "spring", damping: 28, stiffness: 260 }}
            className="w-[440px] max-w-[100vw] flex flex-col h-full bg-white dark:bg-[#0D1526] border-l border-slate-200 dark:border-white/[.07] shadow-2xl overflow-hidden relative"
          >
            <div className="absolute top-0 left-0 w-64 h-64 bg-[#2563EB]/5 dark:bg-[#2563EB]/10 blur-3xl rounded-full pointer-events-none -translate-x-1/2 -translate-y-1/2" />
            <div className="absolute bottom-0 right-0 w-48 h-48 bg-[#F4B400]/5 dark:bg-[#F4B400]/8 blur-3xl rounded-full pointer-events-none translate-x-1/3 translate-y-1/3" />
            ...
            <div className="sticky top-0 z-20 shrink-0 bg-white/95 dark:bg-[#0D1526]/95 backdrop-blur-xl border-b border-slate-200/80 dark:border-white/[.07] px-6 pt-5 pb-4 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0 space-y-1.5">
                    <h2 className="text-[15px] font-black text-slate-900 dark:text-white tracking-tight truncate leading-none">{lead.name}</h2>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-mono text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-white/8 border border-slate-200 dark:border-white/10 px-2 py-0.5 rounded-md tracking-wider">{lead.lead_id || "—"}</span>
              ...
              <div className="flex items-center gap-1 p-1.5 rounded-[14px] bg-slate-100/80 dark:bg-white/[.05] border border-slate-200/60 dark:border-white/[.07] shadow-inner">
                {TABS.map(tab => (
            ...
            {/* SCROLLABLE CONTENT */}
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 text-xs font-sans softphone-scrollbar">
              ...
              {activeTab === "disposition" && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="space-y-4">
                  {user?.role === "agent" && lead.status !== "new" ? (
                    ...
                    <form onSubmit={handleSaveDisposition} className="bg-white dark:bg-[#131F35] rounded-[18px] p-5 border border-slate-200/80 dark:border-white/[.07] shadow-[0_2px_16px_rgba(15,23,42,0.06)] space-y-4">
            ...
            {/* STICKY FOOTER */}
            <div className="sticky bottom-0 z-20 shrink-0 bg-white/95 dark:bg-[#0D1526]/95 backdrop-blur-xl border-t border-slate-200/80 dark:border-white/[.07] px-6 py-4">
              <div className="grid grid-cols-5 gap-2">
                <button onClick={handleCall} title="Call Lead" className="h-10 flex items-center justify-center gap-1.5 rounded-[10px] text-[11px] font-extrabold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/25 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white hover:border-transparent hover:shadow-[0_4px_14px_rgba(5,150,105,0.4)] active:scale-95 transition-all duration-200 cursor-pointer">
                  <PhoneCall className="h-3.5 w-3.5 shrink-0" /><span>Call</span>
                </button>
                <button onClick={() => setShowWhatsAppModal(true)} title="WhatsApp" className="h-10 flex items-center justify-center gap-1.5 rounded-[10px] text-[11px] font-extrabold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/25 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white hover:border-transparent hover:shadow-[0_4px_14px_rgba(5,150,105,0.4)] active:scale-95 transition-all duration-200 cursor-pointer">
                  <MessageSquare className="h-3.5 w-3.5 shrink-0" /><span>WA</span>
                </button>
                <button onClick={() => setShowEmailModal(true)} title="Email" className="h-10 flex items-center justify-center gap-1.5 rounded-[10px] text-[11px] font-extrabold bg-blue-50 dark:bg-[#2563EB]/10 text-[#2563EB] dark:text-[#3B82F6] border border-blue-200 dark:border-[#2563EB]/25 hover:bg-[#2563EB] hover:text-white dark:hover:bg-[#2563EB] dark:hover:text-white hover:border-transparent hover:shadow-[0_4px_14px_rgba(37,99,235,0.45)] active:scale-95 transition-all duration-200 cursor-pointer">
                  <Mail className="h-3.5 w-3.5 shrink-0" /><span>Email</span>
                </button>
                <button onClick={() => handleSaveDisposition()} disabled={isSubmitting || (user?.role === "agent" && lead.status !== "new")} title="Save Disposition" className="h-10 flex items-center justify-center gap-1.5 rounded-[10px] text-[11px] font-extrabold bg-gradient-to-r from-[#1D4ED8] to-[#2563EB] text-white shadow-[0_4px_14px_rgba(37,99,235,0.4)] hover:from-[#1E40AF] hover:to-[#1D4ED8] hover:shadow-[0_6px_18px_rgba(37,99,235,0.55)] active:scale-95 transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none">
                  {isSubmitting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}<span>Save</span>
                </button>
                <button onClick={onClose} title="Close Drawer" className="h-10 flex items-center justify-center gap-1.5 rounded-[10px] text-[11px] font-extrabold bg-slate-100 dark:bg-white/[.06] text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white active:scale-95 transition-all duration-200 cursor-pointer">
                  <X className="h-3.5 w-3.5" /><span>Close</span>
                </button>
              </div>
            </div>
          </motion.div>
