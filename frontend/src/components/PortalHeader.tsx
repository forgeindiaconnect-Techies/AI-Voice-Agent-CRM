...
export default function PortalHeader({
  ...
  return (
    <div className="relative mb-6 font-sans">
      <div className="bg-white/95 backdrop-blur-md px-5 py-3.5 rounded-[18px] shadow-xs border border-slate-200/80 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3.5 transition-all duration-200">
        ...
        {/* Right: Inline Tabs & Action Buttons */}
        <div className="flex items-center gap-2.5 flex-wrap w-full lg:w-auto shrink-0">
          {tabs && tabs.length > 0 && (
            <div className="flex bg-slate-100/90 p-1 rounded-xl border border-slate-200/80">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 text-xs font-extrabold rounded-lg transition duration-200 cursor-pointer ${
                    activeTab === tab.id
          ...
          {primaryButton && (
            <button
              ...
              className="h-9 px-4 bg-[#F4B400] hover:bg-amber-400 text-[#0F4C9A] font-extrabold text-xs rounded-xl transition flex items-center justify-center gap-1.5 shadow-xs active:scale-95 cursor-pointer"
            >
